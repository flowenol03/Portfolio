...
const Skills = () => {
  ...
  const visibleSkills = skills.slice(currentPage * skillsPerPage, (currentPage + 1) * skillsPerPage);

  const nextPage = () => setCurrentPage((prev) => (prev + 1) % totalPages);
  const prevPage = () => setCurrentPage((prev) => (prev - 1 + totalPages) % totalPages);

  return (
    <div className="min-h-screen bg-gradient-to-b from-black to-gray-900 text-white py-20 px-4 flex flex-col items-center">
      ...
      <div className="relative w-full max-w-7xl">
        ...
        <div className={`grid ${isMobile ? 'grid-cols-1 place-items-center' : 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4'} gap-6 px-4`}>
            ...
            {visibleSkills.map((skill, index) => (
              ...
              >
                <Tilt tiltMaxAngleX={10} tiltMaxAngleY={10} className="w-full">
                  <div className="bg-gray-900/70 border-2 border-purple-500 rounded-2xl p-6 text-center shadow-xl hover:shadow-purple-500/50 transition-all duration-300 h-full flex flex-col justify-between">
                    <div>
        ...
        {Array.from({ length: totalPages }).map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-all ${
              currentPage === index ? 'bg-purple-500 w-6' : 'bg-gray-600'
            }`}
          />
        ))}
      </div>
